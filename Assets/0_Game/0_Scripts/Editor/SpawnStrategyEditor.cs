...
[CustomEditor(typeof(SpawnStrategy))]
public class SpawnStrategyEditor : Editor
{
    public override Texture2D RenderStaticPreview(string assetPath, Object[] subAssets, int width, int height) {
        SpawnStrategy itemData = (SpawnStrategy)target;

        // Safety check to ensure target and sprite strategy icons exist
        if (itemData == null || itemData.enemyStrategy == null || itemData.enemyStrategy.Icon == null) {
            return base.RenderStaticPreview(assetPath, subAssets, width, height);
        }

        Sprite sprite = itemData.enemyStrategy.Icon;
